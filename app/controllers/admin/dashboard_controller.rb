class Admin::DashboardController < AdminController
  def index
    @posts_in_digest_count = Post.published.digest.last_day.count
  ...
  def digest
    posts = Post.published.digest.last_day.ranked
    generator = ArticlePdfGenerator.new(posts, date: I18n.l(Time.zone.now, format: :short))

    respond_to do |format|
      format.pdf { send_data generator.render, filename: "Asia-Insight-Briefing-#{ I18n.l Date.today }.pdf" }
    end
  end
